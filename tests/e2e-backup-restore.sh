#!/bin/bash
# End-to-end tests for the homeassistant-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-homeassistant}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-homeassistant-traefik-letsencrypt-docker-compose.yml}"
...
: "${HOMEASSISTANT_BACKUPS_PATH:=/srv/homeassistant/backups}"
: "${HOMEASSISTANT_BACKUP_NAME:=homeassistant-database-backup}"
: "${HOMEASSISTANT_DATA_BACKUP_NAME:=homeassistant-data-backup}"
: "${HOMEASSISTANT_BACKUP_INTERVAL:=24h}"

BACKUPS_PATH="${HOMEASSISTANT_BACKUPS_PATH%/}"
DB_PREFIX="${HOMEASSISTANT_BACKUP_NAME}"
DATA_PREFIX="${HOMEASSISTANT_DATA_BACKUP_NAME}"
INTERVAL="${HOMEASSISTANT_BACKUP_INTERVAL}"
DB_FILE="home-assistant_v2.db"
DB_BASE="home-assistant_v2"
...
APP_CONTAINER="$(docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT_NAME" ps -aq homeassistant | head -n 1)"
...
test_restore_roundtrip() {
  ...
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./homeassistant-restore-data.sh "$stamp" > /dev/null \
    || { fail "./homeassistant-restore-data.sh failed"; return 1; }
  local exists
...
echo "=== homeassistant: backup/restore E2E tests ==="
