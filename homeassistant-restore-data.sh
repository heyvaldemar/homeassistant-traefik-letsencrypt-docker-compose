#!/usr/bin/env bash
# homeassistant-restore-data.sh [timestamp]
#
# Replaces Home Assistant's data with one backup set: the data archive and, for each
# SQLite database, the consistent copy the backups service took beside it.
#
#   ./homeassistant-restore-data.sh                      list the sets and ask
#   ./homeassistant-restore-data.sh YYYY-MM-DD_hh-mm     restore that one
#
...
#
# WHAT THE ARCHIVE LEAVES OUT ON PURPOSE IS KEPT. The loop excludes home-assistant.log* and .cache
# from the archive; those are set aside, the rest is cleared and unpacked, and
# they are put back.
#
# CI runs this exact file.
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than homeassistant.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-homeassistant}"
STOP_SERVICES="homeassistant"                   # every service that writes what is restored
ROOT="/data"                            # where the archive unpacks
PARTS="."                          # what it holds, relative to ROOT
KEEP="home-assistant.log* .cache"                            # what it leaves out on purpose, relative to ROOT
DBS="home-assistant_v2.db"                              # SQLite files in /data, restored from their copies
...
DIR="$(env_of HOMEASSISTANT_BACKUPS_PATH)"; DATA_NAME="$(env_of HOMEASSISTANT_DATA_BACKUP_NAME)"
DB_NAME=""; [ -z "$DBS" ] || DB_NAME="$(env_of HOMEASSISTANT_BACKUP_NAME)"
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; the data may be incomplete. Restore another set before using Home Assistant." >&2
  exit 1
fi
