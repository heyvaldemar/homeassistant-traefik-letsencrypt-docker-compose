...
# # homeassistant-restore-data.sh Description
# Restores homeassistant's data directory from a backup set taken by the backups service.
...
# 3. **Stop Service**: homeassistant is stopped so nothing writes to the data directory.
...
# 5. **Start Service**: homeassistant is started again.
# Make it executable once: `chmod +x homeassistant-restore-data.sh`

APP_CONTAINER="$(docker compose -p homeassistant ps -q homeassistant)"
BACKUPS_CONTAINER="$(docker compose -p homeassistant ps -q backups)"
BACKUP_PATH="/srv/homeassistant/backups"
DB_NAME="homeassistant-database-backup"
DATA_NAME="homeassistant-data-backup"
